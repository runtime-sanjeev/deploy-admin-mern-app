...
function Employee() {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <button onClick={() => viewDoc(row)} className="btn-view-doc">View Document</button>
      ),
      sortable: false,
    },
    {
      name: 'Uploaded Image',
      cell: (row) => (
        <button onClick={() => viewImage(row)} className="btn-view-image">View Image</button>
      ),
      sortable: false,
    },
    {
      name: 'Action',
      cell: (row) => (
        <button onClick={(e) => handleEdit(row, e)} className="btn-edit">Edit</button>
      ),
      sortable: false,
    },
  ];
